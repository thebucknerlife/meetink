...
meetings_log_rebuild() {
...

    {
        ...
        for f in "${kept[@]}"; do
            ...
            # ID-named folders keep the raw stamp as the basename; the
            # human title lives in meta.json — use it for the heading.
            local mtitle=""
            [[ -f "${f:h}/${base}.meta.json" ]] && mtitle=$(python3 -c "
import json, sys
try:
    print(json.load(open(sys.argv[1])).get('title') or '')
except Exception:
    pass" "${f:h}/${base}.meta.json" 2>/dev/null)
